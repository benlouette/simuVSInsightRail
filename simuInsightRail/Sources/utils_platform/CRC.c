 ...
 *      CRC alculation code, as used in the HDLC communication
 *
 *      software implementation based on RFC 1662
 *      Can be adapted to use the on chip CRC hardware device
 */
...
static inline uint16_t fcs_fcs16(uint16_t fcs, uint8_t data)
{
...
static uint16_t fcs_calc(const uint8_t* p,  uint32_t size)
{
    uint16_t    fcs ;
